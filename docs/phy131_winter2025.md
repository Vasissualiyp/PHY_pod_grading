...
[FILES]
file_in = Practical_2/Pods.xlsx
file_out = Practical_2/Marks.csv
...
1. Create a directory with files for that practical, i.e. `Practical_2`
2. In `file_in` and `file_out` in `config.txt`, change directories to what your directory name is
3. Change the name of the assignment in `config.txt` (`PRA_name`) variable. MUST be i.e. `PRA3` (otherwise Quercus won't recognize)
4. Place `Pods.xlsx` into your practical directory, i.e. `Practical_2`.
5. Run `Marking.py` with `python Marking.py` or using a play button in GUI. It should create a `Marks.csv` file.
6. Check that marks make sense!
7. Upload the `Marks.csv` to Quercus in ` Grades > Import `
